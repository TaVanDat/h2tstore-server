...
module.exports = function () {
    ...
    this.addNews = async function (newData, result) {
        const sqlString = "INSERT INTO Category (Code,Name,StatusId,Image) VALUES(@code,@name,@statusId,@image)";
        const pool = await conn
        return pool.request()
    ...
    this.update = async function (id, newData, result) {
        const sqlString = "update Category set Code=@code, Name = @name, StatusId = @statusId,Used = @used, Image = @image where Id = @id";
        const pool = await conn
        return pool.request()
            .input('id', sql.BigInt, id)
            .input('code', sql.NVarChar, newData.Code)
